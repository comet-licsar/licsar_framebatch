...
if [ $STORE -eq 1 ]; then
 ...
 echo "bsub2slurm.sh -w $frame'_geo' -q $bsubquery -n 1 -W 06:00 -M 8000 -o LOGS/framebatch_$frame'_store.out' -e LOGS/framebatch_$frame'_store.err' -J $frame'_ST' store_to_curdir.sh $frame $deleteafterstore" >> framebatch_06_geotiffs_nowait.sh #$frame
 #cd -
fi
