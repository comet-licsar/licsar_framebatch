#run it in frame directory
#it will generate a file with noesd.txt epochs list
out=noesd.txt
echo "searching for problematic ESD"
#rm $out 2>/dev/null
for x in `ls log/coreg_q*.log`; do 
 epoch=`echo $x | rev | cut -d '.' -f2 | cut -d '_' -f1 | rev`
 if [ `grep az_ovr_iteration_1 $x | grep -c "0.00000 " 2>/dev/null` -gt 0 ]; then
  echo $epoch >>$out
 fi
 if [ `wc -l $x | gawk {'print $1'}` -lt 2 ]; then 
  echo $epoch >>$out
 fi; 
done
echo "done, now checking secondary noESDs"
for rslc in `cat $out`; do
