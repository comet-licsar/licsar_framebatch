...
if [ `datediff $s $closestr 1` -lt 180 ]; then postprocflag=''; break; fi
     ...
     if [ `ls SLC.missingbursts 2>/dev/null | wc -l` -gt 0 ]; then
       ...
       echo "we will not attempt for their coregistration - you can do this manually by:"
       #if [ $PROC == 1 ]; then
       #  echo "we will force coregister them now"
       echo "mv SLC.missingbursts/* SLC/."
       echo "framebatch_postproc_coreg.sh -F "$frame" 1"
       #  exit
       #fi
     fi
    if [ $rslcdates -lt 2 ]; then todel=1;
     ...
     #    fi
      ...
      # fix the tmp.tif problem:
      for pr in `ls $frame/GEOC | grep ^20`; do
        if [ ! -f $frame/GEOC/$pr/$pr.geo.unw.tif ]; then
          if [ -f $frame/GEOC/$pr/$pr.geo.unw.tif.tmp.tif ]; then
            echo "fixing unw.tmp.tif of "$pr
            mv $frame/GEOC/$pr/$pr.geo.unw.tif.tmp.tif $frame/GEOC/$pr/$pr.geo.unw.tif
          fi
        fi
      done
